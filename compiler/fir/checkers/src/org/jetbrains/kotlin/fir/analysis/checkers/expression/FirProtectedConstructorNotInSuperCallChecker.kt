...
object FirProtectedConstructorNotInSuperCallChecker : FirFunctionCallChecker() {
    override fun check(expression: FirFunctionCall, context: CheckerContext, reporter: DiagnosticReporter) {
        ...
        if (
            ...
            symbol.visibility.isFormOfProtected &&
            // Prevent reporting for already invisible references
            !reference.isError() &&
            context.containingDeclarations.none { it.symbol == symbol.getConstructedClass(context.session) }
        ) {
    ...
    private fun shouldAllowSuchCallNonetheless(symbol: FirConstructorSymbol, context: CheckerContext): Boolean {
        ...
        return symbol.origin.isJavaOrEnhancement && symbol.callableId.packageName == containingFile.packageFqName
    }
}