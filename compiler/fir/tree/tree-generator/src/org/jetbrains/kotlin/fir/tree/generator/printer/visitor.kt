...
// For not generated fir elements which don't have corresponding 'visit*' and 'transform*' methods we should generate such methods explicitly.
// In particular FirStubStatement and FirDeclarationStatusImpl are not generated and are inherited only from FirPureAbstractElement and
// some interfaces. Since all 'accept' and 'transform' methods was removed from interfaces, we should add them to visitors and transformers.
// In the future we should get rid of this hack, it requires some not trivial refactoring around FirDeclarationStatus.
