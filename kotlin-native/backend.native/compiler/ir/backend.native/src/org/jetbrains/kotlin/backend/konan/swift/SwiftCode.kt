...
sealed interface Swift {
    ...
    interface Builder {
        infix fun String.of(value: Swift.Expression) = Argument.Named(this, value)

        val String.variable: Swift.Expression.Variable
            get() = Swift.Expression.Variable(this)

        val String.type: Swift.Type.Nominal
            get() = Swift.Type.Nominal(this)

        val String.literal: Swift.Expression.StringLiteral
            get() = Swift.Expression.StringLiteral(this)

        val Number.literal: Swift.Expression.NumericLiteral
            get() = Swift.Expression.NumericLiteral(this)
    ...
    interface StatementsBuilder: Builder {
        fun <T : Statement> T.state(): T
    }

    interface ExpressionBuilder: Builder {
        fun <T : Expression> T.compute(): T
    }
    ...
    sealed interface Declaration : Statement {
        enum class Visibility(val displayName: String): Swift {
            PRIVATE("private"),
            ...
            fun renderAsPrefix(parent: Swift?): String = takeIf { it != Visibility.INTERNAL }?.render(parent)?.let { "$it "} ?: ""
        }
        ...
        data class Function(
            ...
            override fun render(parent: Swift?): String {
                val parameters = parameters.map { it.render(this).trim() }.joinToString(separator = ", ").let { "($it)" }
                return listOfNotNull(
        ...
        data class StoredVariable(
            ...
            class Builder(val observers: MutableList<Observer> = mutableListOf()) {
                fun willSet(arg: String? = null, body: Swift.CodeBlock.Builder.() -> Unit): Observer {
                    return Observer("willSet", argumentName = null, Swift.CodeBlock(body)).also { observers.add(it) }
                }

                fun didSet(arg: String? = null, body: Swift.CodeBlock.Builder.() -> Unit): Observer {
                    return Observer("didSet", argumentName = null, Swift.CodeBlock(body)).also { observers.add(it) }
                }
            }
        ...
        data class ComputedVariable(
            ...
            class Builder(val observers: MutableList<Accessor> = mutableListOf()) {
                fun get(body: Swift.CodeBlock.Builder.() -> Unit): Accessor {
                    return Accessor("get", argumentName = null, Swift.CodeBlock(body)).also { observers.add(it) }
                }

                fun set(arg: String? = null, body: Swift.CodeBlock.Builder.() -> Unit): Accessor {
                    return Accessor("set", argumentName = null, Swift.CodeBlock(body)).also { observers.add(it) }
                }
            }
    ...
    sealed interface Statement : Swift {
        data class Assign(val receiver: Swift.Expression, val value: Expression): Statement {
            override fun render(parent: Swift?): String {
    ...
    sealed interface Expression : Statement, Argument, Condition {
        ...
        data class Access(val receiver: Swift.Expression?, val name: String): Expression {
            override fun render(parent: Swift?): String {
                val receiver = receiver?.render(parent)?.trim() ?: ""
        ...
        data class Call(val receiver: Expression, val arguments: List<Argument> = emptyList()): Expression {
            override fun render(parent: Swift?): String {
                val receiver = receiver.render(parent).trim()
                val arguments = arguments.render(parent)
        ...
        data class ArrayLiteral(val value: List<Swift.Expression>) : Expression {
            override fun render(parent: Swift?): String = value.joinToString(separator = ", ") {
                it.render(parent).trim()
            }.let { "[$it]" }
        }

        data class DictionaryLiteral(val value: Map<Swift.Expression, Swift.Expression>) : Expression {
    ...
    sealed interface Type : Swift {
        ...
        data class Tuple(val types: List<Type> = emptyList()) : Type {
            override fun render(parent: Swift?): String {
                return types.map { it.render(parent).trim() }.joinToString(separator = ", ").let { "($it)" }
            }
        }

        data class Function(val arguments: List<Type> = emptyList(), val result: Type?) : Type {
            override fun render(parent: Swift?): String {
                val arguments = arguments.map { it.render(parent).trim() }.joinToString(separator = ", ")
        ...
        data class Existential(val protocols: List<String> = emptyList()) : Type {
            override fun render(parent: Swift?): String = protocols.takeIf { it.isNotEmpty() }?.joinToString(separator = " & ")?.let { "any $it" } ?: "Any"
        }
    }
    ...
    class File : Swift {
        class Builder(
            ...
            override fun <T : Declaration> T.declare(): T = also { this@Builder.declarations.add(it) }
        }
        ...
        override fun render(parent: Swift?): String {
            val imports = imports.map { it.render(this).trim() }.joinToString(separator = "\n", postfix = "\n\n")
            val declarations = declarations.map { it.render(this).trim() }.joinToString(separator = "\n\n")
    ...
    class CodeBlock : Swift {
        ...
        ) : StatementsBuilder, DeclarationsBuilder, ExpressionBuilder {
            override fun <T : Declaration> T.declare(): T = also { this@Builder.statements.add(this) }

            override fun <T : Statement> T.state() = also { this@Builder.statements.add(it) }

            override fun <T : Expression> T.compute(): T = also { this@Builder.statements.add(this) }
        }
        ...
        override fun render(parent: Swift?): String = statements
                .map { it.render(parent).trim() }
                .joinToString(separator = "\n")
    ...
    data class Attribute(val name: String, val arguments: List<Argument>? = null): Swift {
        override fun render(parent: Swift?): String {
            return "@" + Expression.Variable(name).render(parent) + (arguments?.render(parent)?.let { "($it)" } ?: "")
        }
    }
}
...
private fun List<Swift.Attribute>.render(parent: Swift?, separator: String = "\n", terminator: String = "\n"): String? {
    return takeIf { it.isNotEmpty() }
            ?.map { it.render(parent).trim() }
            ?.joinToString(separator = separator, postfix = terminator)
}

private fun List<Swift.Argument>.render(parent: Swift?, separator: String = ", "): String? {
    return takeIf { it.isNotEmpty() }
            ?.map { it.render(parent).trim() }
...
fun Swift.ExpressionBuilder.variable(name: String) = Swift.Expression.Variable(name)

fun Swift.ExpressionBuilder.access(name: String) = Swift.Expression.Access(null, name)
...
inline fun <T> MutableList<Swift.Declaration>.add(body: Swift.DeclarationsBuilder.() -> T): T {
    val builder = (object : Swift.DeclarationsBuilder {
        ...
        override fun <T : Swift.Declaration> T.declare(): T = also { declarations.add(it) }
    })
